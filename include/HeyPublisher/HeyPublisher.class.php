...
EOF;
  ...
  * @since 3.0.0 operates on a JSON version of the author object
  */
  public function update_author_info($uid,$a) {
    ...
    if (FALSE != $a['urls']['website']) {
      // we don't care about errors at this stage
      wp_update_user( array( 'ID' => $uid, 'user_url' => $a['urls']['website'] ) );
    }
    ...
    if ((!$meta2) ||($meta2 != sprintf("%s",$a['id']))) {
      $this->update_author_meta($uid,HEYPUB_USER_META_KEY_AUTHOR_OID,sprintf("%s",$a['id']));
    }
  }
  ...
  */
  public function get_author_id($a) {
    ...
    if (function_exists('get_users')) {
      // First look by OID
      $users = get_users(array('meta_key'=> HEYPUB_USER_META_KEY_AUTHOR_OID, 'meta_value' => $a['id']));
      if (FALSE != $users) {
        $uid = $users[0]->ID;
  ...
  */
  public function create_author($username,$a) {
    ...
    } elseif (($meta2) && ($meta2 != sprintf("%s",$a['id']))) {
